...
for (i in seq_along(all_cluster_sequences)){
  ...
  saveRDS(enrich_result, 
          paste("/enadisk/maison/morlon/stage/results/go_enrichment_analysis_v2_dotplot/GO_analysis", org_name, names(all_cluster_sequences)[i], ".rds", sep = ""))
  # save bar plot
  tryCatch({
    svg_file <- paste("/enadisk/maison/morlon/stage/results/go_enrichment_analysis_v2_dotplot/GO_analysis", org_name, names(all_cluster_sequences)[i], ".svg", sep = "")
    svg(svg_file)
